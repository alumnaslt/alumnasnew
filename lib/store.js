...
async function reserveSlotForCheckout({ mentorId, slotId, studentName, studentEmail }) {
  return withStateLock(async () => {
    ...
    if (!isSlotInFuture(slot)) {
      ...
      throw new Error("Šis laikas jau pasibaigė. Pasirink kitą.");
    }
...
async function markBookingMeetingDetails({
  sessionId,
  ...
  return withStateLock(async () => {
    ...
    booking.googleCalendarEventId = googleCalendarEventId || booking.googleCalendarEventId;
    booking.meetingProvider = meetingProvider || booking.meetingProvider;
    booking.meetingUrl = meetingUrl || booking.meetingUrl;
    booking.meetingCreatedAt = meetingCreatedAt || booking.meetingCreatedAt;
